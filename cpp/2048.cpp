...
std::string Progress(board_t s) {
  ...
  for (int i = 0; i < wholeWidth; i++)
    bar += "█";
  bar += partChar;
  for (int i = 0; i < WIDTH - wholeWidth - 1; i++)
    bar.push_back(' ');
  bar.push_back(']');
  return bar;
}
...
int main(int argc, char *argv[]) {
  ...
  for (int game = 1; game <= iterations; ++game) {
    ...
    for (;;) {
      ...
      if (detailed) {
        std::cout << "Progress:" << std::setw(6) << (1 << maxTile) << ' '
                  << Progress(board) << ' ' << (1 << (maxTile + 1)) << '\r';
        std::cout.flush();
      }
    }
    std::cout << '\n';
  ...
  fout << '\n';
  for (int i = 0; i < 5; ++i)
    fout << (float)bigTiles[i] * 100.0 / (float)iterations << "%,";
  fout << "\n,\nGame,Score,Moves,Time,Speed\n";
  float sumScore = 0, sumMoves = 0, sumTime = 0, sumSpeed = 0;
  for (int i = 0; i < iterations; ++i) {
    fout << i + 1 << ',' << resultScore[i] << ',' << resultMoves[i] << ','
         << resultTime[i] << ',' << resultSpeed[i] << '\n';
    sumScore += resultScore[i];
    sumMoves += resultMoves[i];
    sumTime += resultTime[i];
    sumSpeed += resultSpeed[i];
  }
  fout << "Average" << ',' << sumScore / iterations << ','
       << sumMoves / iterations << ',' << sumTime / iterations << ','
       << sumSpeed / iterations;
}
